...
def main():
    ...
    tx = sqlContext.read.load("/data/bitcoin/transactions.csv", format='com.databricks.spark.csv', inferSchema=True, header=True)

    transaction_value = vout_column.join(tx, vout_column.hash == tx.tx_hash, 'inner').select('tx_hash', 'time', 'publicKey', 'value')
    
    #list of ransomware wallet IDs
    list_of_ransomware = [("CryptoLocker", "{1KP72fBmh3XBRfuJDMn53APaqM6iMRspCh}"),
    ("CryptoLocker", "{1Mz7153HMuxXTuR2R1t78mGSdzaAtNbBWX}"),
    ("CryptoLocker", "{149w62rY42aZBox8fGcmqNsXUzSStKeq8C}"),
    ("CryptoLocker", "{1AjZPMsnmpdK2Rv9KQNfMurTXinscVro9V}"),
    ("CryptoLocker", "{12t9YDPgwueZ9NyMgw519p7AA8isjr6SMw}"),
    ("CryptoLocker", "{13AM4VW2dhxYgXeQepoHkHSQuy6NgaEb94}"),
    ("CryptoLocker", "{115p7UMMngoj1pMvkpHijcRdfJNXj6LrLn}"),
    ...
    ransomware_df = sqlContext.createDataFrame(list_of_ransomware).toDF("ransomware", "pubkey")

    # Join matching with vout transaction to retrieve donors publicKeys and bitcoins sent
    ransomware_affected_wallets = transaction_value.join(ransomware_df, ransomware_df.pubkey == transaction_value.publicKey , 'inner').select('tx_hash', 'time', 'pubkey', 'value').orderBy('value', ascending=False)

    second_joins = ransomware_affected_wallets.select('tx_hash', 'time', 'pubkey',ransomware_affected_wallets.value.cast("float").alias('values'))
   
    second_joins.rdd.coalesce(1).saveAsTextFile("output")
    second_joins.write.format('com.databricks.spark.csv').save('transaction.csv')

    top10Wallet = second_joins.groupBy('pubkey').sum('values').orderBy('sum(values)', ascending=False)

    top10Wallet.rdd.saveAsTextFile("outSum")
    top10Wallet.write.format('com.databricks.spark.csv').save('walletSum.csv')

    walletTransactions = second_joins.groupBy('pubkey').count().orderBy('count', ascending=False)

    walletTransactions.rdd.saveAsTextFile("walletTransactionsCount")
    walletTransactions.write.format('com.databricks.spark.csv').save('walletTransactionsCount.csv')
